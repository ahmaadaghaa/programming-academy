...
// 4. Check for existing user (username, email, OR phone if provided)
try {
    // بناء الاستعلام ديناميكياً لتجنب تطابق الأرقام الفارغة
    $where = ["email = ?", "username = ?"]; 
    $params = [$email, $username];
    // لا نتحقق من الهاتف إلا إذا كان مُقدماً وغير فارغ
    if ($phone !== null && $phone !== '') {
        $where[] = "phone = ?";
        $params[] = $phone;
    }
    $sqlCheck = "SELECT COUNT(*) FROM users WHERE " . implode(' OR ', $where);
...
try {
    ...
    $stmt->execute([
        ...
        $lastName, 
        $email, 
        // خزن NULL بدلاً من سلسلة فارغة للهاتف لتفادي التعارضات الخاطئة
        ($phone === '' ? null : $phone), 
        $username, 
